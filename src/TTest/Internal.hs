...
data TTTrace = TTTrace !Class !(Traces.Trace Float)
  deriving (Eq, Show)
...
ttest :: Monad m => ConduitT TTTrace Void m (U.Vector Float)
ttest = getZipSink (go <$> population0 <*> population1)
  where
...
foldStats :: Stats           -- ^ initial state
          -> TTTrace         -- ^ the new trace
          -> Stats           -- ^ the new state
foldStats (Stats 0 _ _) (TTTrace _ x) = Stats 1 x zeros
  where
    zeros = U.map (const 0) x -- Assuming the two vectors have the same size
foldStats (Stats n m s) (TTTrace _ x) = Stats (n+1) m' s'
  where
